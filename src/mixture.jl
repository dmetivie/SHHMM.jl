...
function fit_em(mix::MixtureModel, y; display = :none, maxiter = 100, tol = 1e-3, robust = false)
    ...
    [LL[:, k] = log(α[k]) .+ logpdf.(dists[k], y) for k = 1:K]
    ...
    for it = 1:maxiter
        ...
        α[:] = mean(γ, dims = 1)
        dists[:] = [fit_mle(types[k], y, γ[:, k]) for k = 1:K]
...
function model_for_dist(dist::Exponential, θ_Y::AbstractArray, γ::AbstractVector, y, n2t, T::Int; silence = true)
    ...
    # Polynomial P
    @NLexpression(model, Pθ[n = 1:N], sum(trig[n][j] * θ_jump[j] for j = 1:(2d+1)))
    ...
    @NLobjective(
        model, Max,
        -sum(πk[n] * (Pθ[n] + R[n] * exp(-Pθ[n])) for n = 1:N)
    )
...
function model_for_dist(dist::Gamma, θ_Y::AbstractArray, γ::AbstractVector, y, n2t, T::Int; silence = true)
    ...
    # Polynomial P
    @NLexpression(model, Pθ[n = 1:N], sum(trig[n][j] * θ_jump[j] for j = 1:length(trig[n])))
    @NLexpression(model, Pa[n = 1:N], sum(trig[n][j] * a_jump[j] for j = 1:length(trig[n])))
    ...
    @NLobjective(
        ...
        -sum(πk[n] * (exp(Pa[n]) * Pθ[n] + R[n] * exp(-Pθ[n]) + loggamma(exp(Pa[n])) - expm1(Pa[n]) * log(R[n])) for n = 1:N)
    )
...
function model_for_α(θ_α::AbstractMatrix, γ::AbstractMatrix, n2t, T::Int; silence = true)
    ...
    # Polynomial P
    @NLexpression(model, Pn[n = 1:N, k = 1:K-1], sum(trig[n][j] * θ_jump[k, j] for j = 1:length(trig[n])))

    @NLparameter(model, πk[n = 1:N, k = 1:K] == γ[n, k])
    @NLobjective(
        model,
        Max,
        sum(sum(πk[n, k] * Pn[n, k] for k = 1:K-1) - log1p(sum(exp(Pn[n, k]) for k = 1:K-1)) for n = 1:N) #Σ_k πk[n,k] = 1
    )
    # I don't know if it is the best but https://discourse.julialang.org/t/jump-updating-nlparameter-of-a-model-in-a-loop/35081/3
    model[:πk] = πk
    return model
...
function update_α!(
    ...
    [set_value(πk[n, k], γ[n, k]) for n = 1:N, k = 1:K]
    ...
    [α[K, t] = 1 for t = 1:T] # last colum is 1/normalization (one could do otherwise)
...
function update_dist(dist::AbstractVector{F} where {F<:Gamma}, θ_Y::AbstractArray, γ::AbstractVector, observations, model; warm_start = true)
    ...
    a = [exp(polynomial_trigo(t, θ_Y[1, :], T = T)) for t = 1:T]
    p = [exp(polynomial_trigo(t, θ_Y[2, :], T = T)) for t = 1:T]
    dist[:] = [Gamma(a[t], p[t]) for t = 1:T]
    return dist, θ_Y
...
function model_for_dist_d(dist::Gamma, θ_Y::AbstractArray, T::Int; silence = true)
    ...
    # Polynomial P
    @NLexpression(model, Pθ[t = 1:T], sum(trig[t][j] * θ_jump[j] for j = 1:length(trig[t])))
    @NLexpression(model, Pa[t = 1:T], sum(trig[t][j] * a_jump[j] for j = 1:length(trig[t])))
    ...
    @NLobjective(
        ...
        -sum(s_π[t] * exp(Pa[t]) * Pθ[t] + s_π_R[t] * exp(-Pθ[t])
             + s_π[t] * loggamma(exp(Pa[t])) - s_π_logR[t] * expm1(Pa[t]) for t = 1:T)
    )
...
function fit_mle(mix::Vector{F} where {F<:MixtureModel}, θ_α::AbstractArray, θ_Y::AbstractArray, y, n2t;
    ...
    model_dist = [model_for_dist_d(dists[k][1], θ_Y[k], T; silence = silence) for k = 1:K]
...
function fit_em_GE(mix::Vector{F} where {F<:MixtureModel}, θ_α::AbstractArray, θ_Y::AbstractArray, y, n2t;
    ...
    model_dist = [model_for_dist(dists[k][1], θ_Y[k], γ[:, k], y, n2t, T; silence = silence) for k = 1:K]
